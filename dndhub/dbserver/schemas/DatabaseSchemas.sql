...
create table if not exists OptionSet (
    ...
    foreign key (equip)
    references EquipmentCategory(equipment_category),
    foreign key (resourceList)
    references ResourceList(resource_list_url),
    foreign key (optArray_id)
    references OptionsArray(array_id)
)
...
create table if not exists Background (
    ...
    foreign key (ability_scores)
    references ArrayAPIReference(array_id),
    foreign key (feat)
    references BackgroundFeatReference(index),
    foreign key (proficiencies)
    references ArrayAPIReference(array_id),
    foreign key (proficiency_choices)
    references ArrayChoice(id),
    foreign key (equipment_options)
    references ArrayChoice(id)
)
